...
function insertUser($params){
	...
	$sql .= " VALUES ('".trim($params['username'])."', '".password_hash($params['password'], PASSWORD_DEFAULT)."', '{$params['firstname']}', '{$params['lastname']}', '{$params['email']}', '{$params['phone']}', '". date('Y-m-d', strtotime($params['dob'])) ."', '{$params['gender']}', '{$params['profile']}' )";
	//echo $sql; exit;
	return $pdo->exec($sql);
}
function fetchUsers(){
